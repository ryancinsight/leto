...
iterator/slicing surface, companion crates) to a complete count.
...
| Symmetric eigen (values+vectors) | `SymmetricEigen` | `symmetric_eigen_jacobi`, `symmetric_eigen_qr` / `SymmetricEigenWorkspace` | Verified | eigen.rs; symmetric_qr.rs (closed forms, backward-error residuals, Jacobi agreement) |
| Symmetric eigenvalues-only | `SymmetricEigen::eigenvalues` | `symmetric_eigenvalues_jacobi` | Verified | oracle_parity.rs |
| Thin SVD, rank-revealing (incl. rank-deficient U/V) | `SVD` | `svd_decompose` / `MatrixDecompose::svd` | Verified | svd/bidiagonal_qr — Golub–Reinsch implicit-shift QR with U/V (const-generic `VEC`); the sole SVD path since ADR 0005 retired one-sided Jacobi. Rank deficiency is returned as `σ = 0`, never an error; `UᵀU = VᵀV = I` holds at deficient rank. Oracles: reconstruction + orthonormality across tall/square/wide, sub-tolerance σ reconstruction, closed-form diagonal σ, cross-path σ agreement vs the values-only instantiation |
...
| Pseudo-inverse (full-rank **and** rank-deficient) | `pseudo_inverse` | `pinv` / `MatrixSolve::pinv` | Verified | svd/pseudoinverse.rs — `A⁺ = V Σ⁺ Uᵀ` on `svd_decompose` under a relative `τ·σ_max` cutoff; Moore-Penrose `A A⁺ A = A`, `A⁺ A A⁺ = A⁺` on rank-deficient input |
