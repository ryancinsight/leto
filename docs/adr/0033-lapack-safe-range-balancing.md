...
`dbdsqr` with `dlasv2` 2×2 blocks; both gates take LAPACK's driver range;
...
`col_piv_qr`) formed products of entries that overflow or underflow long
before the input leaves the scalar's range: f64 SVD failed beyond
`2^±256`, `RealSchur::eigenvalues` returned `{3, 3}` for `{2, 4}` at `2⁻⁸⁶`
in f32, f64 `col_piv_qr` returned rank 0 beyond `2^±512`, and F16/Bf16
Francis stalled. Balancing the whole matrix alone does not fix it: local
products depend on local magnitudes during the iteration, and a fixed
landing scales entries far below the largest into underflow.
...
false in F16. ⁴ Recursive sums stagnate in F16 (`2048 + 1 = 2048`); a
...
| Francis | Hessenberg dots `‖v‖₂·‖A‖_F`, `‖v‖₂ ≤ 4√n`; driver range | 2 | `2r` | `⌈log₂ n⌉` |
| Golub–Kahan | reflector dots `‖v‖₂·‖A‖_F`, `‖v‖₂ ≤ 4√M`; driver range | 2 | `2 + ⌈⌈log₂ M⌉/2⌉ + r` | `⌈log₂ k⌉` |
...
costs `γ₅₃ ≈ 0.026`/`0.21`. Every format is certified a posteriori
...
- `dbdsqr`'s `6k²·safmin` (and `dlahqr`'s absolute `safmin·n/ulp`):
  assume `n²·unfl ≪ ulp` / `ulp² ≫ safmin`, false in F16. `6k²·safmin` is
...
`schur.rs` — the clustered family at the gate edge, and the Ahues–Tisseur,
pre-check and `tst = 0` decisions; `eigen.rs` — Jacobi exact on 295
diagonals per format. In-range results are not bit-identical to the
pre-change tree. `pinv` reports `Overflow` when a retained singular value's
...
scale_range, graded_scan, schur, eigen}.rs`.
