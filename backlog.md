...
- [ ] [patch] Fix `batched_matmul`'s parallel closure full-buffer `&mut` aliasing: each task materializes `from_raw_parts_mut(out_ptr, out_len)` over the whole output and writes only its batch sub-region — runtime-disjoint but Tree-Borrows UB to hold N concurrent full-range `&mut`. Borrow only each batch's disjoint sub-slice (or a raw-pointer view that never forms a full `&mut`). Needs Tree-Borrows/Miri verification. (Per-row `parallel_dot/cc/outer` kernels are already disjoint and clean.)
