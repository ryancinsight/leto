...
- Current consumer: Apollo-owned cold failures leave Leto `633acb7` unchanged. Executable 6,861,312 bytes (-512 baseline), format, all-target/all-feature Clippy and 15 focused debug/release tests pass at 311 unchanged inputs. [Evidence](../../output/apollo-square-transpose/cold-failures/apollo-final-checks.json); normalized movement and caller locations are preserved. Full consumer gates and timing remain pending; [first census attempt](../../output/apollo-square-transpose/cold-failures/census/plan.json) aborts before native invocation due to active Cargo processes, yielding no measurements.
