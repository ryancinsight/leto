...
/// Thin singular value decomposition of a tall or square full-column-rank matrix.
///
/// The decomposition stores `A = U * diag(singular_values) * V^T`, where
/// `U` has shape `m x n`, `V` has shape `n x n`, and singular values are sorted
/// in descending order. This first SVD increment deliberately targets the
/// full-column-rank least-squares/PCA shape used by current Atlas consumers.
/// Wide or rank-deficient matrices are rejected instead of returning fabricated
/// vectors for null-space components.
#[derive(Debug, Clone)]
...
/// Compute a thin SVD for a tall or square full-column-rank matrix.
///
/// The implementation is generic over [`RealScalar`] and runs in the native
/// precision of `T`: it forms `A^T A`, decomposes that symmetric matrix through
/// the existing Jacobi eigensolver, and derives `U = A V Σ^-1`. The method is
/// appropriate for small dense matrices and for consumer migration tests; it is
/// not a replacement for a rank-revealing bidiagonal SVD on ill-conditioned
/// large matrices.
...
/// Compute a thin SVD with an explicit rank and eigensolver tolerance.
pub fn svd_decompose_with_tolerance<T: RealScalar>(
    ...
    for new_col in 0..cols {
        ...
        if eigenvalue < T::ZERO {
            if eigenvalue.neg() > tolerance {
                return Err(LetoError::StorageError {
                    reason: "SVD normal matrix has a negative eigenvalue beyond tolerance"
                        .to_string(),
                });
            }
            return Err(LetoError::StorageError {
        ...
        normalize_left_column(&mut left_values, rows, cols, new_col, tolerance)?;
    }
...
/// Return singular values for a tall or square full-column-rank matrix.
...
fn gram_matrix<T: RealScalar>(matrix: &ArrayView2<'_, T>) -> Result<Array2<T>> {
    let [rows, cols] = matrix.shape();
...
fn normalize_left_column<T: RealScalar>(
    values: &mut [T],
    rows: usize,
    cols: usize,
    col: usize,
    ...
    if norm <= tolerance {
        return Err(LetoError::StorageError {
            reason: "SVD left singular vector has zero norm".to_string(),
        });
    }
    for row in 0..rows {
