...
use crate::application::zip::zip2_mut_with;
    ...
    T: RealField + FloatElement + Copy,
{
    ...
    let field_lo = field
        ...
        .unwrap();
    zip2_mut_with(&mut dst_int, &field_hi, &field_lo, |r, &hi, &lo| {
        *r = (hi - lo) * inv_2h
    })
    .unwrap();
    ...
    T: RealField + FloatElement + Copy,
{
    ...
    zip2_mut_with(&mut dst_bot, &field_hi_bot, &field_lo_bot, |r, &hi, &lo| {
        *r = (hi - lo) * inv_h
    })
    .unwrap();
    let mut dst_top = dst
    ...
    let field_lo_top = field
        ...
        .unwrap();
    zip2_mut_with(&mut dst_top, &field_hi_top, &field_lo_top, |r, &hi, &lo| {
        *r = (hi - lo) * inv_h
    })
    .unwrap();
    Ok(())
    ...
    T: RealField + FloatElement + Copy,
{
    ...
    let field_lo_far = field
        ...
        .unwrap();
    zip2_mut_with(&mut dst_far, &field_hi_far, &field_lo_far, |r, &hi, &lo| {
        *r = (hi - lo) * inv_h
    })
    .unwrap();
    Ok(())
