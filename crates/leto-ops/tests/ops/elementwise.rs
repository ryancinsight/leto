...
use leto_ops::{
    add, binary_map, div, indexed_zip_many_mut_with, indexed_zip_mut_with, map, map_into, mapv,
    mul, scalar_map, sub, unary_map, zip_mut_with, AddOp, EqOp, ErfOp, ErfcOp, GeOp, GtOp, LeOp,
...
#[test]
fn test_indexed_zip_many_mut_with_handles_strided_transposed_views() {
    let layout = Layout::c_contiguous([2, 3]).unwrap();
    ...
    indexed_zip_many_mut_with(&mut out, (&a_t, &b_t), |[row, col], left, (av, bv)| {
